...
class lambda_test : public ::testing::Test {
 protected:
  struct Mock {
    Mock(int ret) : ret{ ret } {}

    template <class Slice, class ... Args>
    auto& slice(Slice, Args... ) const {
      return ret;
    }
...
TEST_F(lambda_test, when_no_ids_and_view_doesnt_have_arguments_view_is_called) {
  LeL::Lambda<LeL::Context<LeL::Identity, Mock>, LeL::Template::Box<char>> lambda(Mock(5));
...
TEST_F(lambda_test, when_no_ids_view_can_be_called_with_arguments) {
  LeL::Lambda<LeL::Context<LeL::Identity, Mock>, LeL::Template::Box<char, '0'>> lambda(Mock(7));
